...
addition.  For the ed25519 and curve25519 curves the private keys are no longer
multipliers for the group generator; instead the hash of the private
key is the multiplier.  For this reason, our scheme for ed25519 and curve2559 does
not support public key derivation and uses the produced hashes directly
as private keys.

For ed25519 public keys we define ser<sub>P</sub>(P): serializes the elliptic curve point P = (x,y) on a twisted Edwards curve as a byte sequence: 0x00 || ENC(x, y), where ENC is defined in RFC 8032.

For curve25519 public keys we define ser<sub>P</sub>(P): serializes the elliptic curve point P = (u,v) on a Montgomery curve as a byte sequence: 0x00 || encodeUCoordinate(u, 255), where encodeUCoordinate is defined in RFC 7748.

For ed25519 and curve25519 private keys we define ser<sub>256</sub>(p) = p, since private keys for these two curves are not integers but byte sequences.
...
* Curve = "curve25519 seed" for the curve25519 curve.

For ed25519 and curve25519, the last step always succeeds since every 256-bit number
...
Child key derivation in BIP-0032 may result in an invalid key, in which case
the child key for the given index is undefined and one should proceed with the
next index value. For the secp256k1 curve the probability of this happening is
lower than 2<sup>&minus;127</sup>, i.e. practically impossible. For the
NIST P-256 curve, on the other hand, the probability is 2<sup>&minus;32</sup>.
The present specification extends the BIP-0032 definition of child key
derivation so that the child keys for all indices are well defined. The reason
for extending the definition is to avoid undefined child keys and complicated
error handling when dealing with the NIST P-256 curve. However, the extended
definition also applies to the secp256k1 curve.
...
* Chain m/0<sub>H</sub>
  ...
  * public: 07992b3f270ef15f266785fffb73246ad7f40d1fe8679b737fed0970d92cc5f39
* Chain m/0<sub>H</sub>/2147483647<sub>H</sub>
  * fingerprint: 0x5b26da66
